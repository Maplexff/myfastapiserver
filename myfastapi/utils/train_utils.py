...
my训练
'''
def mytrain(model, runner, lr_update_func, device, epoch, epoches, test_cfg, meta):
    train_loss = 0
    pred_list, target_list = [], []
    runner['epoch'] = epoch + 1
    meta['epoch'] = runner['epoch']
...
def myvalidation(model, runner, cfg, device, epoch, epoches, meta):
    pred_list, target_list = [], []
    val_loss = 0.0
    model.eval()
    with torch.no_grad():
    ...
    # meta['train_info']['val_acc0'].append(eval_results)
    # meta['train_info']['val_acc1'].append(eval_results)
    # meta['train_info']['val_acc2'].append(eval_results)
    # meta['train_info']['val_acc3'].append(eval_results)
    # meta['train_info']['val_acc4'].append(eval_results)
    # meta['train_info']['val_acca'].append(eval_results)
